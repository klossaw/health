...
pdf("./combine/updata_score1_boxplot.pdf")
...
ic <- vm
pdf("./combine/updata_score1x_boxplot.pdf")
my_comparisons <- list(c("normal", "level1"), c("level1","level2"), c("normal","level2"))
...
pdf("./combine/updata_score2_boxplot.pdf",width = 10, height = 10)
for(i in ic){
  mx <- 3
...
# 2401
pdf("./combine/level_barplot.pdf",width = 12)
for(i in v2){
  p11 <- ggplot(boxsub3_core_frame, aes(x = .data[[i]], fill = .data[["right_level"]])) +
    geom_bar(position = "fill") + theme_classic() + myggtitle(boxsub3_core_frame[[i]], boxsub3_core_frame[["right_level"]])
  p12 <- ggplot(boxsub3_core_frame, aes( x = .data[[i]], fill = .data[["left_level"]])) +
    geom_bar(position = "fill") + theme_classic()+ myggtitle(boxsub3_core_frame[[i]], boxsub3_core_frame[["left_level"]])
  p13 <- ggplot(boxsub3_core_frame, aes(x = .data[[i]], fill = .data[["max_level"]])) +
    geom_bar(position = "fill") + theme_classic()+ myggtitle(boxsub3_core_frame[[i]], boxsub3_core_frame[["max_level"]])
  
  p21 <- ggplot(boxsub3_core_frame, aes(x = .data[[i]], fill = .data[["right_level"]])) +
    geom_bar() + theme_classic()
  p22 <- ggplot(boxsub3_core_frame, aes( x = .data[[i]], fill = .data[["left_level"]])) +
    geom_bar() + theme_classic()
  p23 <- ggplot(boxsub3_core_frame, aes(x = .data[[i]], fill = .data[["max_level"]])) +
    geom_bar() + theme_classic()
  
  print((p11 + p12 + p13)/(p21 + p22 + p23))
}
dev.off()
...
# 2382
pdf("./combine/zhifanggan_level_barplot.pdf",width = 12)
i = v2
p11 <- ggplot(boxsub3x_core_frame, aes(x = .data[[i]], fill = .data[["right_level"]])) +
  geom_bar(position = "fill") + theme_classic() + myggtitle(boxsub3x_core_frame[[i]], boxsub3x_core_frame[["right_level"]]) + 
  xlab("zhifanggan")
p12 <- ggplot(boxsub3x_core_frame, aes( x = .data[[i]], fill = .data[["left_level"]])) +
  geom_bar(position = "fill") + theme_classic()+ myggtitle(boxsub3x_core_frame[[i]], boxsub3x_core_frame[["left_level"]]) + 
  xlab("zhifanggan")
p13 <- ggplot(boxsub3x_core_frame, aes(x = .data[[i]], fill = .data[["max_level"]])) +
  geom_bar(position = "fill") + theme_classic()+ myggtitle(boxsub3x_core_frame[[i]], boxsub3x_core_frame[["max_level"]]) + 
  xlab("zhifanggan")

p21 <- ggplot(boxsub3x_core_frame, aes(x = .data[[i]], fill = .data[["right_level"]])) +
  geom_bar() + theme_classic() + 
  xlab("zhifanggan")
p22 <- ggplot(boxsub3x_core_frame, aes( x = .data[[i]], fill = .data[["left_level"]])) +
  geom_bar() + theme_classic() + 
  xlab("zhifanggan")
p23 <- ggplot(boxsub3x_core_frame, aes(x = .data[[i]], fill = .data[["max_level"]])) +
  geom_bar() + theme_classic() + 
  xlab("zhifanggan")
print((p11 + p12 + p13)/(p21 + p22 + p23))
dev.off()
