...
# 2401
pdf("./fix_first/core_distribution_fill_barplot.pdf",width = 12)
for(i in v2){
  p1 <- ggplot(boxsub3_core_frame, aes(x =  NL_level, fill = .data[[i]] %>% as.character())) +
    geom_bar(position = "fill") + theme_classic() + 
    myggtitle(boxsub3_core_frame[[i]], boxsub3_core_frame[["NL_level"]]) +
...
pdf("./fix_first/core_distribution_count_barplot.pdf",width = 12)
for(i in v2){
  p1 <- ggplot(boxsub3_core_frame, aes(x =  NL_level, fill = .data[[i]] %>% as.character())) +
...
# 2382
pdf("./fix_first/core_distribution_zhifanggan_fill_barplot.pdf",width = 12)
i = v2
p1 <- ggplot(boxsub3x_core_frame, aes(x =  NL_level, fill = .data[[i]] %>% as.character())) +
  geom_bar(position = "fill") + theme_classic() + 
  myggtitle(boxsub3x_core_frame[[i]], boxsub3x_core_frame[["NL_level"]]) + 
  xlab("zhifanggan") +
  facet_grid( ~  xingbie_level) + 
  scale_fill_viridis_d() + 
  guides(fill = guide_legend(title=i))
...
pdf("./fix_first/core_distribution_zhifanggan_count_barplot_count.pdf",width = 12)
i = v2
p1 <- ggplot(boxsub3x_core_frame, aes(x =  NL_level, fill = .data[[i]] %>% as.character())) +
  geom_bar() + theme_classic() + 
  myggtitle(boxsub3x_core_frame[[i]], boxsub3x_core_frame[["NL_level"]]) + 
...
pdf("./fix_first/face_distribution_boxplot.pdf", height = 12, width = 16)
for(i in ic){
...
ic <- vm
pdf("./fix_first/face_distribution_boxplotx.pdf", height = 12, width = 16)
for(i in 1:length(ic)){
  ...
  p1 <- ggboxplot(sub_frame, x = "NL_level", y = i, fill = "NL_level") +
    stat_compare_means(method = "anova") + NoLegend() +
    facet_grid( ~ xingbie_level)  + ylab(yn) +
