...
dir.create("./secondtime/")
ic <- c("NHDL", "LDL", "NHDL_NL", "LDL_NL", "shousuoya", "shuzhangya", "bmi", "fuwei")
pdf("./secondtime/updata_score1_boxplot.pdf")
...
ic <- vm
pdf("./secondtime/updata_score1x_boxplot.pdf")
my_comparisons <- list(c("normal", "level1"),c("level1","level2"),c("normal","level2"))
for(ix in 1:length(ic)){
  i = ic[[ix]]
  yn <- names(ic)[[ix]]
  p1 <- ggboxplot(boxsub1x_core_frame, x = "left_level", y = i, fill = "left_level",
                  palette = c("#00AFBB", "#E7B800", "#FC4E07")) +
    stat_compare_means(comparisons = my_comparisons) + NoLegend() + ylab(yn)
  p2 <- ggboxplot(boxsub1x_core_frame, x = "right_level", y = i, fill = "right_level",
                  palette = c("#00AFBB", "#E7B800", "#FC4E07")) +
    stat_compare_means(comparisons = my_comparisons) + NoLegend() + ylab(yn)
  p3 <- ggboxplot(boxsub1x_core_frame, x = "max_level", y = i, fill = "max_level",
                  palette = c("#00AFBB", "#E7B800", "#FC4E07")) +
    stat_compare_means(comparisons = my_comparisons) + NoLegend()+ ylab(yn)
  p4 <- ggboxplot(boxsub1x_core_frame %>% dplyr::filter(!is.na(total_level)), x = "total_level",
...
pdf("./secondtime/updata_score2_boxplot.pdf",width = 10, height = 10)
for(i in ic){
  mx <- 3
...
library(patchwork)
# 2400
pdf("./secondtime/level_barplot.pdf",width = 12)
for(i in v2){
...
pdf("./secondtime/core_pointplot.pdf")
for(i in v2){
...
ic <- vm
pdf("./secondtime/other_pointplot.pdf")
for(ix in 1:length(ic)){
  i = ic[[ix]]
  yn <- names(ic)[[ix]]
  p1 <- ggscatter(point_other_frame, x = i, y = "right", add = "reg.line") +
    stat_cor(label.y = 7, aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
    stat_regline_equation(label.y = 6) + xlab(yn)
  p2 <- ggscatter(point_other_frame, x = i, y = "left", add = "reg.line") +
    stat_cor(label.y = 7, aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
    stat_regline_equation(label.y = 6) + xlab(yn)
  p3 <- ggscatter(point_other_frame, x = i, y = "max_data", add = "reg.line") +
    stat_cor(label.y = 7, aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
    stat_regline_equation(label.y = 6) + xlab(yn)
  p4 <- ggscatter(point_other_frame, x = i, y = "total_level", add = "reg.line") +
    stat_cor(label.y = 18, aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
